...
export default function ThumbnailUpload({
  ...
  const borderClass = error
    ...
    : dragOver
      ...
      : "border-slate-300 hover:border-primary/50 hover:bg-primary/[0.03]";

  const containerClass = previewUrl
    ? `border-2 border-solid bg-white shadow-sm ${error ? "border-red-300" : "border-slate-200"}`
  ...
  return (
    <div className="w-full shrink-0 lg:w-[180px]" data-form-field="thumbnail">
      ...
      >
        {previewUrl ? (
          <>
            ...
            />
            <span className="pointer-events-none absolute left-2.5 top-2.5 rounded-lg bg-slate-900/80 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white backdrop-blur-sm">
              Thumbnail
            </span>
            <button
              type="button"
              onClick={onRemove}
              className="absolute right-2 top-2 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-white/95 text-slate-500 shadow-md transition hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 lg:opacity-0 lg:group-hover:opacity-100"
              aria-label="Remove thumbnail"
            >
              <Trash2 className="h-3.5 w-3.5" />
            </button>
            <div className="absolute inset-0 flex items-end justify-center bg-gradient-to-t from-slate-900/75 via-slate-900/20 to-transparent pb-3 opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-within:opacity-100">
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-800">Upload Thumbnail</p>
              <p className="mt-0.5 text-[11px] text-slate-500">JPG, PNG, WebP</p>
              <p className="text-[10px] text-slate-400">Max {MAX_THUMBNAIL_MB} MB</p>
            </div>
      ...
      {error ? (
        ...
        <p className="mt-2 text-xs text-slate-400">Main image in search & listings</p>
      )}
    </div>
  );
}
