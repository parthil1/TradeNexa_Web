          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
            ...
            <div className="space-y-6 lg:col-span-5">
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-[#1a2b4c]">Our Office</h3>
                {offices.map((office, idx) => (
                  <div
                    key={idx}
                    className="space-y-3 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm"
                  >
                    <h4 className="flex items-center gap-2 text-sm font-bold text-[#1a2b4c]">
                      <MapPin className="h-4 w-4 text-primary" />
                      {office.city}
                    </h4>
                    <p className="pl-6 text-xs leading-relaxed text-slate-500">{office.address}</p>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                ...
                <div className="space-y-3 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                  <h4 className="text-sm font-bold text-[#1a2b4c]">Connect Socially</h4>
                  <div className="flex gap-3">
                    {["facebook", "twitter", "linkedin", "instagram"].map((social) => (
                      <span
                        key={social}
                        className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-400 transition hover:border-primary/30 hover:text-primary"
                        aria-label={social}
                      />
                    ))}
                  </div>
